...
const TvShows = () => {
  ...
  const [movie, setMovie] = useState({ title: "Cargando programas de tv..." });
  const [searchKey, setSearchKey] = useState("");

  const fetchTv = async (searchKey) => {
  ...
  return (
    <div className="container">
      ...
      <div class="container mt-2">
        <div class="row ">
          {movies.map((tvShows) => (
            <Link to={`/detailTvShows/${tvShows.id}}`}>
              <div key={tvShows.id} className="col-md-4">
                <img
                  src={`${IMAGE_PATH + tvShows.poster_path}`}
                  alt=""
                  height={600}
                  width="100%"
                  onClick={() => {
